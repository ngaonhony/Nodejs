...
const Landlord = ({ user, posts }) => {
  const filteredPosts = posts?posts.filter(p => String(p.userId._id) === String(user._id)):1;
  const postCount = filteredPosts.length;
  const createdAtFormatted = format(new Date(user.createdAt), 'dd/MM/yyyy');
  return (
    <div>
      <div className="w-20 h-20 rounded-full bg-gray-200 mx-auto mb-2"><img
        src={user.avatar || "https://www.bing.com/th?id=OIP.Fogk0Q6C7GEQEdVyrbV9MwHaHa&w=206&h=206&c=8&rs=1&qlt=90&o=6&dpr=1.3&pid=3.1&rm=2"} // Đường dẫn ảnh mặc định nếu không có ảnh
        alt="Avatar"
