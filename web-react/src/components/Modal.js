...
import { getNumbersPrice, getNumbersArea } from '../ultils/getNumbers';
...
const Modal = ({ setIsShowModal, content, name, handleSubmit, queries, arrMinMax, defaultText }) => {
    ...
    return (
        <div onClick={() => { setIsShowModal(false); }} className='fixed top-0 left-0 right-0 bottom-0 bg-overlay-70 z-20 flex justify-center items-center'>
            <div onClick={(e) => { e.stopPropagation(); setIsShowModal(true); }} className='w-2/5 h-[500px] bg-white rounded-md relative'>
                <div className='h-[45px] px-4 flex items-center border-b border-gray-200'>
                    <span className='cursor-pointer' onClick={(e) => { e.stopPropagation(); setIsShowModal(false); }}>
                        ...
                        <div className='flex flex-col items-center justify-center relative'>
                            <div className='z-30 absolute top-[-48px] font-bold text-xl text-orange-600'>
                                {(persent1 === 100 && persent2 === 100) 
                                    ...
                                    : `Từ ${Math.min(convert100toTarget(persent1), convert100toTarget(persent2))} - ${Math.max(convert100toTarget(persent1), convert100toTarget(persent2))} ${name === 'price' ? 'triệu' : 'm2'}`}
                            </div>
                            ...
                            <div className='absolute z-30 top-6 left-0 right-0 flex justify-between items-center'>
                                <span className='cursor-pointer' onClick={(e) => { e.stopPropagation(); handleClickTrack(e, 0); }}>0</span>
                                <span className='mr-[-12px] cursor-pointer' onClick={(e) => { e.stopPropagation(); handleClickTrack(e, 100); }}>
